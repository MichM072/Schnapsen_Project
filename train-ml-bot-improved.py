...
def create_training_bot():
    name = options.training_bot
...
def create_dataset(path, player=rdeep.Bot(), games=500, phase=1):
    features = create_training_bot()

    if options.player != rdeep.Bot():
        player = util.load_player(options.player)

    if options.games != 500:
...
parser.add_argument("-p", "--player",
                    dest="training_bot",
                    help="Choose which bot is being trained",
                    default="ml")

parser.add_argument("-e", "--enemy",
                    dest="player",
                    help="Choose which bot it uses to train",
                    default="rdeep")
