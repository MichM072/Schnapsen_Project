...
parser.add_argument("-p", "--player",
                    dest="player",
                    help="Input the name of bot for training.",
                    default="rand")

parser.add_argument("-mlb", "--machinelearningbot",
                    dest="mlbot",
                    help="Input the name of which ml bot you want to import the features.",
                    default="ml")

parser.add_argument("-g", "--games",
                    dest="games",
                    help="Input the amount of games you would like to train.",
                    default="2000")

options = parser.parse_args()

def create_features():
    """Retrieve the features of the desired ml bot
    The flag mlb defines which bot we we use to retrieve the feature set.
    The default setting is ml bot, so it will always be able to retrieve features.
    """
    name = options.mlbot
...
def create_dataset(path, player=util.load_player(options.player), games=int(options.games), phase=2):
...
if options.overwrite or not os.path.isfile(options.dset_path):
    create_dataset(options.dset_path, player=util.load_player(options.player), games=int(options.games))
...
if options.train:
    ...
    joblib.dump(model, "./bots/{}/".format(options.mlbot) + options.model_path)
